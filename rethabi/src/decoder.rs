...
fn decode_param(
    ...
    match *param {
        ParamType::Address => {
            ...
            let result =
                DecodeResult { token: Token::Address(address.into()), new_offset: offset + 32 };
            Ok(result)
        }
        ParamType::Int(_) => {
            let slice = peek_32_bytes(data, offset)?;
...
mod tests {
    ...
    fn decode_static_tuple_of_addresses_and_uints() {
        ...
        let decoded = decode(
            &[ParamType::Tuple(vec![ParamType::Address, ParamType::Address, ParamType::Uint(32)])],
            &encoded,
        )
        .unwrap();
    ...
    fn decode_dynamic_tuple() {
        ...
        let decoded =
            decode(&[ParamType::Tuple(vec![ParamType::String, ParamType::String])], &encoded)
                .unwrap();
        let expected = vec![tuple];
        assert_eq!(decoded, expected);
    }
